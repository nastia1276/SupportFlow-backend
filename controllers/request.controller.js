...
exports.createRequest = async (req, res) => {
  try {
    const { category_id, title, description, budget, priority, city, region } =
      req.body;
    ...
    const [volunteers] = await db.query(
      "SELECT telegram_id FROM users WHERE role='volunteer' AND telegram_id IS NOT NULL"
    );
    ...
    for (const volunteer of volunteers) {
      await notifyUser(volunteer.telegram_id, message);
    }
  ...
  try {
    ...
    if (existingRequest.status !== "Очікує на виконавця") {
      return res.status(400).json({
        success: false,
        message:
          "Можна редагувати тільки запити зі статусом 'Очікує на виконавця'",
      });
    }
  ...
  try {
    ...
    if (existingRequest.status !== "Очікує на виконавця") {
      return res.status(400).json({
        success: false,
        message:
          "Можна взяти в роботу тільки запити зі статусом 'Очікує на виконавця'",
      });
    }
    ...
    if (userRow && userRow[0] && userRow[0].telegram_id) {
      await notifyUser(
        ...
        `Ваше замовлення "${existingRequest.title}" взяв у роботу волонтер!`
      );
    }
...
exports.updateRequestStatus = async (req, res) => {
  try {
    ...
    if (
      !status ||
      !["Очікує на виконавця", "В роботі", "Виконано", "Скасовано"].includes(
        status
      )
    ) {
      return res.status(400).json({
        success: false,
        message:
          "Невірний статус. Допустимі значення: 'Очікує на виконавця', 'В роботі', 'Виконано', 'Скасовано'",
      });
    }
    ...
    if (req.user.role === "requester") {
      ...
      if (status !== "Скасовано" && status !== "Виконано") {
        return res.status(403).json({
          success: false,
          message:
            "Замовники можуть тільки скасувати запит або позначити його як виконаний",
        });
      }
      ...
      if (status === "Очікує на виконавця") {
        return res.status(403).json({
          success: false,
          message:
            "Волонтери не можуть змінювати статус на 'Очікує на виконавця'",
        });
      }
    }
    ...
    if (userRow && userRow[0] && userRow[0].telegram_id) {
      await notifyUser(
        userRow[0].telegram_id,
        `Статус вашого замовлення "${existingRequest.title}" змінено на: ${status}.`
      );
    }
    if (status === "Виконано" && userRow[0].telegram_id) {
      await notifyUser(
        userRow[0].telegram_id,
        `Ваше замовлення "${existingRequest.title}" позначене як виконане!`
      );
    }
